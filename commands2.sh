...
run_async_main() {
    ...
    if [[ "$post_process" == "True" ]]; then
        python async_main.py --language "$language" --strategy "$strategy" --max_iters "$max_iters" --few_shot "$few_shot" --model "$model_gpt" --post_process "True" > "$out_file"
    else
        python async_main.py --language "$language" --strategy "$strategy" --max_iters "$max_iters" --few_shot "$few_shot" --model "$model_gpt" > "$out_file"
    fi
    end_time=$(date +%s)
    echo "Completed async_main.py with language $language, strategy $strategy, few_shot $few_shot, and model $model_gpt in $((end_time - start_time)) seconds"
}
