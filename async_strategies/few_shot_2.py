...
from prompts import BASIC_EXTRACT_PROMPT_PY, BASIC_EXTRACT_PROMPT_RS, problem_statements_py, problem_statements_rs, problem_solutions_py, problem_solutions_rs, FEW_SHOT_PROMPT_PY
...
class FewShotStrategy2(BaseStrategy):
    ...
    async def run(self, problem_item, language, model_args):
        if language == "py":
            prompt = FEW_SHOT_PROMPT_PY.format(prompt=problem_statements_py[0])
            messages = [{"role": "user", "content": prompt}]
            messages += [{"role": "assistant", "content": problem_solutions_py[0]}]
            for i in range(1, min(model_args.get("few_shot", 1024), len(problem_statements_py))):
                messages.append({"role": "user", "content": FEW_SHOT_PROMPT_PY.format(prompt=problem_statements_py[i])})
                messages.append({"role": "assistant", "content": problem_solutions_py[i]})
            messages.append({"role": "user", "content": FEW_SHOT_PROMPT_PY.format(prompt=problem_item["prompt"])})
        elif language == "rs":
            prompt = BASIC_EXTRACT_PROMPT_RS.format(prompt=problem_statements_rs[0])
            messages = [{"role": "user", "content": prompt}]
            messages += [{"role": "assistant", "content": problem_solutions_rs[0]}]
            for i in range(1, min(model_args.get("few_shot", 1024), len(problem_statements_rs))):
                ...
                messages.append({"role": "assistant", "content": problem_solutions_rs[i]})
